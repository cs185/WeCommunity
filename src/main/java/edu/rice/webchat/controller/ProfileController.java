...
public class ProfileController {
    ...
    @Autowired
    public ProfileController(AccountService accountService, UserService userService, GroupService groupService, FriendshipService friendshipService, UserRepository userRepository) {
    ...
    @GetMapping("/profile")
    public String showProfile(HttpSession session, HttpServletResponse response){
        try {
        if (!accountService.authenticate(session))
                response.sendError(401, "Invalid credential"); // Unauthorized

            response.sendRedirect("/profile/" + session.getAttribute("username"));
        } catch (IOException e) {
    ...
    // todo: put authenticate to every controller method
    @GetMapping("/profile/{username}")
    public ModelAndView showProfile(@PathVariable("username") String username, HttpSession session, HttpServletResponse response) {
        if (!accountService.authenticate(session)) {
            try {
//                response.sendError(401, "Invalid credential"); // Unauthorized
